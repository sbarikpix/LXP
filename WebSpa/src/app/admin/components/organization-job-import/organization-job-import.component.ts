...
@Component({
  selector: 'app-organization-job-import',
  templateUrl: './organization-job-import.component.html',
  styleUrl: './organization-job-import.component.scss'
})
export class OrganizationJobImportComponent implements OnInit {
  ...
  exportSource: any[] = [{ "OrganizationName": "OrganizationName", "JobName": "JobName", "Description": "Description" }];

  constructor(public dialogRef: MatDialogRef<OrganizationJobImportComponent>, private http: HttpClient, public toaster: ToastrService,
    @Inject(MAT_DIALOG_DATA) public data: { userId: string, }, private jobService: JobTitleService) { }
  ngOnInit() {
  ...
  uploadFile(): void {
    ...
    this.jobService.uploadFile(this.selectedFile, this.loggedInUser.applicationUserId).subscribe({
      next: (response) => {
      ...
      error: (err: HttpErrorResponse) => {
        this.onClose();
        let errormsg = err.error['OrganizationJobs'] != null ? err.error['OrganizationJobs'] : err.error['error']
