...
@Component({
  selector: 'app-organization-team-import',
  templateUrl: './organization-team-import.component.html',
  styleUrl: './organization-team-import.component.scss'
})
export class OrganizationTeamImportComponent implements OnInit {
  ...
  exportSource: any[] = [{ "OrganizationName": "OrganizationName", "TeamName": "TeamName", "Description": "Description" }];

  constructor(public dialogRef: MatDialogRef<OrganizationTeamImportComponent>, private http: HttpClient, public toaster: ToastrService,
    @Inject(MAT_DIALOG_DATA) public data: { userId: string, }, private teamService: TeamService) { }
  ...
  uploadFile(): void {
    ...
    this.teamService.uploadFile(this.selectedFile, this.loggedInUser.applicationUserId).subscribe({
      next: (response) => {
      ...
      error: (err: HttpErrorResponse) => {
        this.onClose();
        let errormsg = err.error['OrganizationTeams'] != null ? err.error['OrganizationTeams'] : err.error['error']
