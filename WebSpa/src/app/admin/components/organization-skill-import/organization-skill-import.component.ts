...
@Component({
  selector: 'app-organization-skill-import',
  templateUrl: './organization-skill-import.component.html',
  styleUrl: './organization-skill-import.component.scss'
})
export class OrganizationSkillImportComponent implements OnInit {
  ...
  exportSource: any[] = [{ "JobCategoryName": "Software Development", "SkillName": "Java Software Development", "SkillDescription": "Java Software Development Description" }];

  constructor(public dialogRef: MatDialogRef<OrganizationSkillImportComponent>, private http: HttpClient, public toaster: ToastrService,
    @Inject(MAT_DIALOG_DATA) public data: { userId: string, }, private skillService: SkillService) { }
  ngOnInit() {
  ...
  uploadFile(): void {
    ...
    this.skillService.uploadFile(this.selectedFile, this.loggedInUser.applicationUserId, this.loggedInUser.organizationId).subscribe({
      next: (response) => {
      ...
      error: (err: HttpErrorResponse) => {
        this.onClose();
        let errormsg = err.error['OrganizationSkills'] != null ? err.error['OrganizationSkills'] : err.error['error']
